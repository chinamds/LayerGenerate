...
namespace LayerGen.DatabasePlugins
{
	...
	public class Index : Single, IIndex
	{
        ...
        internal void AddColumn(string dbName, string schema, string table, string constraintName)
        {
            //if (null == _columns)
            //{
            //    _columns = (Columns)this.dbRoot.ClassFactory.CreateColumns();
            ...
            //    _columns.Index = this;
            //}
            String[] indexesRestrictions = new String[5];
            indexesRestrictions[0] = dbName;
            indexesRestrictions[1] = schema;
            indexesRestrictions[2] = table;
            indexesRestrictions[3] = constraintName;
            //indexColumnsRestrictions[4] = "CourseID";
            try
            { 
                DataTable metaData = this.LoadDataSqlClient("IndexColumns", indexesRestrictions);
                DataRow[] rows = metaData.Select("", "ordinal_position asc");
            
                int count = rows.Length;
                for (int i = 0; i < count; i++)
                {
                    _columnNames.Add(rows[i]["COLUMN_NAME"] as string);
                }
            }
            catch (Exception ex)
            {
                throw new Exception(string.Format("Table:{0}, Index '{1}' AddColumn error!", table, constraintName), ex);
            }
        ...
        virtual public string[] ColumnNames
        {
            get
            {
