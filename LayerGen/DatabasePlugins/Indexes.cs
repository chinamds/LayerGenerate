	...
	public class Indexes : Collection, IIndexes, IEnumerable, ICollection
	{
		...
		internal void PopulateArray(DataTable metaData)
		{
			...
			for(int i = 0; i < count; i++)
			{
                ...
                index.AddColumn(rows[i]["table_catalog"] as string, rows[i]["table_schema"] as string, rows[i]["table_name"] as string, rows[i]["INDEX_NAME"] as string);
			}
		}
        ...
        virtual public bool LoadIndexes(string dbName, string schema, string table)
        {
            return true;
        }
